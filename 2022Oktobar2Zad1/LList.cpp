...
void LList::MoveMinToHead(LListNode** start)
{
	LListNode* tmp = head;
	LListNode* spok = NULL;
	LListNode* preStart = NULL;
	LListNode* minEl = *start;
	LListNode* minElSpok = NULL;

	while (tmp != NULL) {
		if (spok != NULL && spok->next == *start) {
			preStart = spok;
		}
		if ((preStart != NULL || *start == head) && tmp->info < minEl->info) {
			minEl = tmp;
			minElSpok = spok;
		}
		spok = tmp;
