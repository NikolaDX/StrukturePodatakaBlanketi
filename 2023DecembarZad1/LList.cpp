...
bool LList::reverseOrder(int first, int last)
{
	...
	LListNode* tmp = head;
	LListNode* spok = NULL;
	LListNode* firstEl = NULL;
	LListNode* preFirst = NULL;
	...
	while (tmp != NULL) {
		...
		if (firstFound && tmp->info == last) {
			LListNode* lastBackup = tmp->next;
			LListNode* nextEl = tmp->next;
			LListNode* firstPointer = firstEl->next;
			LListNode* elBackup = spok;
			tmp = firstEl;
			tmp->next = nextEl;
			nextEl = tmp;
			tmp = firstPointer;
			while (tmp->next != lastBackup) {
				firstPointer = tmp->next;
				tmp->next = nextEl;
				nextEl = tmp;
				tmp = firstPointer;
			}
