...
bool LList::deleteSublist(LList* sublist)
{
	...
	LListNode* tmp = head;
	LListNode* spok = NULL;
	LListNode* backupSpok = NULL;
	int count = 0;

	while (tmp != NULL) {
		if (tmp->info == sublist->head->info) {
			backupSpok = spok;
			LListNode* sublistNode = sublist->head;
			bool found = true;
			while (sublistNode != NULL) {
				if (tmp->info != sublistNode->info) {
					found = false;
					break;
				}
				spok = tmp;
			...
			if (found) {
				LListNode* delTmp = backupSpok->next;
				LListNode* delTmpNxt = delTmp->next;
				for (int i = 0; i < count; i++) {
					delete delTmp;
					delTmp = delTmpNxt;
					delTmpNxt = delTmp->next;
				}
				backupSpok->next = tmp;
				return true;
			}
