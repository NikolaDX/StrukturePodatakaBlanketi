...
bool OpenScatterTable::signup(int playerNumber, char* name, char* lastName, int yearBorn, int jerseyNumber)
{
	char playerNumberStr[20];
	snprintf(playerNumberStr, sizeof(playerNumberStr), "%d", playerNumber);
	ScatterObject obj(playerNumberStr, name);
	ScatterObject defaultObj;
	if (find(playerNumberStr) == defaultObj) {
		insert(obj);
		return true;
	}
	return false;
